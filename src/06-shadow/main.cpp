...
public:
    ...
    bool Initialize() override {
        ...
        launch_params_.light.strength = pcm::Vec3(1000.0f, 1000.0f, 1000.0f);
...
private:
        ...
        for (size_t i = 0; i < meshes_buffers_.size(); i++) {
            for (const OptixProgramGroup pg : hitgroup_pg_) {
                ...
                if (scene_.meshes[i].base_color_map_index >= 0) {
                    rec.data.base_color_mapped = true;
                    rec.data.base_color_map =
                        scene_textures_[scene_.meshes[i].base_color_map_index].CudaObject();
                }
                else {
                    rec.data.base_color_mapped = false;
                }
    ...
    void Update() override {
        launch_params_.light.position = pcm::Vec3(

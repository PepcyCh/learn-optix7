...
OPTIX_CLOSESTHIT(Radiance)() {
    ...
    const uint32_t base_index = mesh_data->index[prim_id * 3];
    const Vec3 &p0 = mesh_data->vertex[base_index];
    const Vec3 &p1 = mesh_data->vertex[base_index + 1];
    const Vec3 &p2 = mesh_data->vertex[base_index + 2];
